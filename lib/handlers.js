...
handlers._users.put = (data, callback) => {
  ...
  if (phone) {
    ...
    if (firstName || lastName || password) {
        ...
        if (tokenIsValid) {
          ...
          _data.read('users', phone, (err, userData) => {
            if (!err && userData) {
              ...
              //Persist the new updates to disk
      ...
      callback(400, { 'Error': 'Dummy, I need atleast one thing to change...' });
    }
  } else {
    callback(400, { 'Error': 'Missing required field!' });
  }
                    ...
                    _data.delete('checks', checkId, (err) => {
                      ...
                      if (checksDeleted === checksToDelete) {
                        if (!deletionErrors) {
                          ...
                          callback(500, { 'Error': 'Errors incountered when trying to delete all user\'s checks. All checks may not have been deleted from the system successfully.' })
                        }
                      }
                    });
                  });
...
handlers._tokens.post = (data, callback) => {
  ...
  if (phone && password) {
    ...
    _data.read('users', phone, (err, userData) => {
      if (!err && userData) {
        //Hash the sent password and compair it to the password stored in the user object.
        let hashedPassword = helpers.hash(password);
        if (hashedPassword === userData.hashedPassword) {
          // If valid create a new token with a random name. Then, set the expiration one hour into the future.
...
handlers._tokens.put = (data, callback) => {
  ...
  if (id && extend) {
    ...
    callback(400, { 'Error': 'Missing required field(s) or field(s) are/is invalid.' });
  }
      ...
      if (!err && tokenData) {
        ...
        _data.read('users', userPhone, (err, userData) => {
          if (!err && userData) {
            ...
            //Verify that the user has less than the max amount of checks per user.
            if (userChecks.length < config.maxChecks) {
              ...
              //Persist the checkObject to disk.
              ...
              callback(400, { 'Error': `The user already has the maximum number of checks. The maximum number of checks allowed is: ${config.maxChecks}` });
            }
          } else {
            callback(403)
          }
  ...
  if (id) {
    ...
    _data.read('checks', id, (err, checkData) => {
      if (!err && checkData) {
        ...
        //Get the token from the headers
        let token = typeof (data.headers.token) === 'string' ? data.headers.token : false;
        //Verify that the given token is valid for the check's creator.
...
handlers._checks.put = (data, callback) => {
  ...
  if (id) {
    //Check to make sure one or more of the optional data is included.
    if (protocol || url || method || successCodes || timeoutSeconds) {
      //Look up the check.
      _data.read('checks', id, (err, checkData) => {
        if (!err && checkData) {
                ...
                _data.read('users', checkData.userPhone, (err, userData) => {
                  if (!err && userData) {
                    ...
                    if (checkPosition > -1) {
                      ...
                      callback(500, { 'Error': 'Could not find the check on the users oject, so could not remove it.' });
                    }
                  } else {
                    callback(400, { 'Error': 'Could not find the user who created the check, so could not remove the check from the user\'s object.' });
                  }
                });
              } else {
